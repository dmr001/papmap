...
function getPages(pdf, first, last) {
  ...
  // allPages = pdf.getPage(1);  // We really only need the first page for our thumbnail

  allPages = pdf.promise.then(function(pdfDoc) {
    return pdfDoc.getPage(1);
  })
  console.log("getPages: allPages:")
...
export default {
  ...
  methods: {
    ...
    fetchPages(currentPage = 0) {
      ...
      console.log(`Fetching pages ${startPage} to ${endPage}, from pdf:`);
      console.log(this.pdf);
      getPages(this.pdf, startPage, endPage)
          .then((pages) => {
            const deleteCount = 0;
            console.log("Splicing pages:")
