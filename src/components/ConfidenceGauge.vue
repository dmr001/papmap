<template>
  <div>
    <v-badge left bordered overlap :content="Math.round(100 * confidence)/100"
             offset-x="20" offset-y="recommendations[scenario].confidence * 10">
  <svg height="50" width="55">
    <defs>
      ...
      <rect v-if="!isNaN(confidence)" x="30" :y="yConfidence" width="8" :height="40 - yConfidence" fill="url(#grad1)" />
      <rect v-if="!isNaN(confidence)" x="30" y="0" width="8" height="40"
           style="fill:gray;stroke:gray;stroke-width:0;fill-opacity:0.2;stroke-opacity:0.9" />

      <rect v-else x="30" y="0" width="8" height="40" fill="url(#diagonalHatch)" />




    <text x="45" y="45" style="fill:black;" font-family="Arial" font-size = "11">0
      <tspan x="45" y="8">1</tspan>
    </text>


      <polygon v-if="!isNaN(confidence)" :points="confidenceTriangle" style="fill:blue;stroke:blue;stroke-width:1" />
...
-->
  ...
  <div v-if="!isNaN(confidence)" class="text-caption" >
    Confidence
  </div>
  ...
  methods: {
    ...
    computed: {
      ...
      confidenceTriangle: function () {
        ...
        let points = `27,${this.yConfidence - 2} 27,${this.yConfidence + 2} 30,${this.yConfidence}`
